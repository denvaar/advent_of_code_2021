defmodule DumboLights do
  def count_flashes(_, 0), do: :never

  def count_flashes(levels, iterations) do
    ...
    all_flashed? =
      levels
      |> Tuple.to_list()
      |> Enum.all?(fn level -> level == 0 end)

    if all_flashed? do
      100_000 - iterations + 1
    else
      count_flashes(levels, iterations - 1)
    end
  end
...
IO.inspect(
  DumboLights.count_flashes(levels, 100_000),
  label: "Result"
)
